...
export default async function PortfolioPage() {
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 pb-16" style={{ paddingTop: "calc(var(--scene-h) + 2.5rem)" }}>
      ...
      {/* Обычная равномерная сетка (grid), не masonry: масонри давало
          ячейки разной высоты и рисковало горизонтальным скроллом.
          Ячейки одного размера, фото обрезаются под рамку (cover) —
          тот же подход, что и в PortfolioFlow (попап поверх 3D-сцены),
          см. content-flow.module.css. */}
      <div className="mt-10 grid grid-cols-2 gap-3 sm:grid-cols-3">
        {items.map((item) => {
          const caption = locale === "ro" ? item.captionRo : item.captionRu;
          return (
            <figure key={item.id} className="group relative aspect-[3/4] overflow-hidden rounded-md">
