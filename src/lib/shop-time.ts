...
// Шаг СЕТКИ стартовых точек времени (не длительность самой услуги — та
// теперь берётся из Service.durationMin, см. /api/booking/slots
// ?serviceId=). 30 минут — компромисс между не слишком редкой сеткой
// (не пропускать реально свободные окна) и не слишком частой (список
// слотов не должен быть длиннее экрана).
